...
/**
 * 結果をWeb Share APIで共有する関数（スマホのLINE等に対応）
 */
async function shareResults() {
    const resultText = generateResultText();

    if (navigator.share) {
        // Web Share API対応（iOS・Androidのネイティブ共有シートを使用）
        try {
            await navigator.share({ title: '精算結果', text: resultText });
        } catch (error) {
        ...
        // PC等のフォールバック：クリップボードにコピー
        shareButton.disabled = true;
        shareButton.innerHTML = '<i class="fas fa-spinner fa-spin"></i> コピー中...';
        try {
            ...
            shareButton.innerHTML = '<i class="fas fa-share-nodes"></i> 結果を共有';
        } finally {
...
/**
 * クリップボードコピー成功時の表示（PCフォールバック用）
 */
function showShareSuccess() {
    shareButton.innerHTML = '<i class="fas fa-check"></i> コピー完了！';
